...
"""
# Critical data path
CRITICAL_DATA_FILE_PATH = Config.PATH_CRITICAL_DATA
...
def df_append_data(df_new:pd.DataFrame, type_name:str) -> int:
    ...
    date_end = datetime(year_max, month_max, calendar.monthrange(year_max,month_max)[1])  
    ...
    while current_date <= date_end:
        ...
        filename=Config.PATH_DATAFRAMES+f'{current_date.year}_{current_date.month:02}_{type_name}.parquet'
        df_add_df_to_parquet_file(filename,df_month)
        
        # Check if both files exist and update/create the critical data file
        filename_trip = Config.PATH_DATAFRAMES+f'{current_date.year}_{current_date.month:02}_trip.parquet'
        filename_charge = Config.PATH_DATAFRAMES+f'{current_date.year}_{current_date.month:02}_charge.parquet'
