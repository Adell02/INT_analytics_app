...
def seeder(email=None,password=None,company=None):
    if email == None:
    ...
    cursor.execute(''' INSERT INTO user (userid,email,password,company,role,is_confirmed) VALUES (0,%s,%s,%s,%s,%s)''',(email,password,company,role,confirmed))
...
def new_table():    
    ...
    cursor.execute(''' CREATE TABLE user (userid INT NOT NULL AUTO_INCREMENT, email VARCHAR(50) NOT NULL, password VARCHAR(100) NOT NULL, company VARCHAR(50) NOT NULL, role VARCHAR(50) NOT NULL DEFAULT 'user', is_confirmed BOOLEAN NOT NULL, PRIMARY KEY(userid)); ''')
    #Saving the Actions performed on the DB
    conn.commit()
